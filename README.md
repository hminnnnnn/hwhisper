...
단축키 탭 → 마이크 캡처 → **온디바이스 STT**(Apple `SpeechTranscriber`, 구형 Mac은 WhisperKit) → 선택적 LLM 정제 → 커서에 삽입. 음성은 기기를 떠나지 않고, 정제할 때도 텍스트만 전송됩니다.

- **삽입** — 클립보드 + ⌘V 기본(범용), 일부 앱은 손쉬운 사용 API로 직접 삽입. 보안 입력 필드는 건너뜁니다.
- **개인 사전** — 인식 힌트 → 정제 보호 → 최종 치환 3단계로 고유명사를 항상 올바르게 표기합니다.
- **정제** — OpenAI 호환 클라이언트 하나로 Gemini/Groq/Ollama/커스텀 지원, 실패·타임아웃 시 원문 삽입.
- **저장** — 히스토리(SQLite)·개인 사전·API 키 모두 로컬(`~/Library/Application Support/Hwhisper/`, 소유자 전용 0600).
...
6. 정제 타임아웃은 따로 설정하지 않습니다 — 내부적으로 20초를 기다리고 최대 3회까지 재시도하며, 그래도 실패하면 다듬지 않은 원문이 대신 삽입됩니다.

**프라이버시:** 정제를 켜면 받아쓰기 원문에서 다듬어진 **텍스트만** 선택한 프로바이더로 전송됩니다(커스텀 프로바이더는 `https://` 또는 로컬 `localhost`만 허용 — 평문 전송 방지). **음성(오디오)은 어떤 경우에도 외부로 전송되지 않습니다** — 음성 인식은 항상 기기 안에서 끝납니다. 참고로 텍스트 삽입은 기본적으로 클립보드+⌘V 방식을 씁니다. **v0.2.14부터는 삽입에 성공한 뒤에도 받아쓴 텍스트가 클립보드에 그대로 남습니다** — 삽입이 엉뚱한 곳에 갔거나 앱이 조용히 삼켰을 때 ⌘V 한 번으로 복구할 수 있게 하기 위해서입니다. 그 대가로 **이전에 복사해 두었던 클립보드 내용은 복원되지 않으며**, 클립보드 히스토리 앱을 쓰신다면 받아쓴 내용이 그곳에 쌓입니다. 보안 입력 필드(비밀번호 등)가 활성화된 경우에는 삽입도 클립보드 쓰기도 하지 않습니다.
