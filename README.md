...
<p align="center">
  <img src="assets/demo.gif" width="720" alt="hwhisper 데모 — 온보딩, 홈 대시보드, 히스토리, 개인 사전, 설정">
</p>
...
- **온디바이스 받아쓰기** — Apple `SpeechTranscriber`(한국어 CER 1.9%)로 음성을 기기 안에서 텍스트로 변환. 구형 Mac은 WhisperKit로 자동 폴백.
- **유연한 단축키** — 단일 키 토글(우측⌘/우측⌥/fn 프리셋, 또는 원하는 보조키·함수키를 직접 지정) 또는 조합키. 녹음 중 X 버튼으로 즉시 취소.
...
각 기능이 실제로 어떻게 구현되는지 요약합니다.

- **딕테이션 파이프라인** — 단축키 탭 → 마이크 캡처(16kHz 모노) → 적응형 VAD로 앞뒤 무음 제거 → **온디바이스 STT**(macOS 26+는 Apple `SpeechTranscriber`, 그 외는 WhisperKit) → (선택) LLM 정제 → 커서에 삽입. 전 과정을 상태 머신(idle→listening→transcribing→refining→inserting→restoring)으로 관리하고, 연속 발화는 큐(depth 3)로 직렬 처리합니다. 발화가 없으면 삽입 대신 "들리지 않았어요" 경고를 띄웁니다.
- **삽입** — 기본은 **클립보드 + ⌘V**(범용). TextEdit/Notes 등 화이트리스트 앱은 손쉬운 사용(Accessibility) API로 직접 삽입하고 read-back으로 검증합니다. 삽입 직전 대상 컨텍스트를 재확인해, 포커스가 바뀌었으면 버리지 않고 클립보드에 보존합니다. 보안 입력 필드(암호 등)로 판단되면 삽입을 건너뛰고 클립보드에도 쓰지 않습니다.
- **텍스트 정제** — 하나의 OpenAI 호환 chat-completions 클라이언트로 Gemini/Groq/Ollama/커스텀을 모두 지원합니다. **정제된 텍스트만** 전송되고 음성은 전송되지 않으며, 커스텀 엔드포인트는 `https://`(또는 로컬 `localhost`)만 허용합니다. 타임아웃(기본 8초) 초과 시 원문이 삽입됩니다.
- **개인 사전 (3단계 방어)** — ① **인식 바이어싱**: 등록 용어를 STT의 `contextualStrings`로 전달해 애초에 맞게 인식. ② **정제 보호**: 원문에 실제로 등장하는 용어만 "수정 금지"로 LLM에 전달(없는 용어를 주면 LLM이 지어내는 문제를 방지). ③ **최종 치환**: 정제 후 잘못된 표기를 올바른 표기로 교체(최장 일치 우선, 영문 대소문자 무시·단어 경계 준수).
- **히스토리** — 로컬 **SQLite**(`~/Library/Application Support/Hwhisper/history.sqlite3`, 0600)에 원본·정제본·대상 앱·발화 길이를 저장하고, 한국어 부분 일치를 위해 FTS 대신 `LIKE` 부분 문자열 검색을 씁니다. 홈 대시보드 지표는 이 데이터에서 이번 주치를 집계합니다.
- **단축키 감지** — 단일 키는 보조키(⌘/⌥/⌃/⇧·fn)의 짧은 "탭"을 `flagsChanged`로, 함수키(F1~F20)는 `keyDown`으로 감지합니다(다른 키와 함께 누르면 무시되어 원래 동작을 방해하지 않음). 조합키는 KeyboardShortcuts 라이브러리를 사용합니다. 전역 감지는 macOS의 입력 모니터링 권한이 필요합니다.
- **프라이버시·저장** — 음성은 어떤 경우에도 외부로 나가지 않습니다. API 키·히스토리·개인 사전은 모두 `~/Library/Application Support/Hwhisper/`에 현재 사용자만 읽을 수 있는 권한(0600)으로 저장됩니다.
